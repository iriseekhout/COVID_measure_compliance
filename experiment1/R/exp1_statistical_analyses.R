...
alpha_behaviour_1 <- dataset %>%
  select(Gedrag_2_1, Gedrag_2_2, Gedrag_2_3, Gedrag_2_4, Gedrag_2_5, Gedrag_2_6, Gedrag_2_7, Gedrag_2_8, Gedrag_2_9) %>%
  psych::alpha()
...
# propensity score
ps <- glm(
  formula = group ~ Geslacht + Leeftijd,
  family = "binomial",
  data = dataset
)
...
# self efficacy T0
m1_SE_0 <- lm(
  formula = SE_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_SE_0 <- lm(
  formula = SE_0 ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_SE_0_ps <- lm(
  formula = SE_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)

qqnorm(m_SE_0_ps$residuals)
qqline(m_SE_0_ps$residuals)



# intention T0
m1_INT_0 <- lm(
  formula = INT_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_INT_0 <- lm(
  formula = INT_0 ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_INT_0_ps <- lm(
  formula = INT_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)
...
# perceived risk (self) T0
m1_KANS_0 <- lm(
  formula = KANS_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_KANS_0 <- lm(
  formula = KANS_0 ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_KANS_0_ps <- lm(
  formula = KANS_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)

qqnorm(m_KANS_0_ps$residuals)
qqline(m_KANS_0_ps$residuals)



# perceived severity T0
m1_PS_0 <- lm(
  formula = Ernst_besmet ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_PS_0 <- lm(
  formula = Ernst_besmet ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_PS_0_ps <- lm(
  formula = Ernst_besmet ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)

qqnorm(m_PS_0_ps$residuals)
qqline(m_PS_0_ps$residuals)



# perceived vulnerability (others) T0
m1_VUL_GRP_0 <- lm(
  formula = VUL_GRP_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_VUL_GRP_0 <- lm(
  formula = VUL_GRP_0 ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_VUL_GRP_0_ps <- lm(
  formula = VUL_GRP_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)

qqnorm(m_VUL_GRP_0_ps$residuals)
qqline(m_VUL_GRP_0_ps$residuals)



# perceived compliance T0
m1_PC_0 <- lm(
  formula = gepercipieerde_compliance ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_PC_0 <- lm(
  formula = gepercipieerde_compliance ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_PC_0_ps <- lm(
  formula = gepercipieerde_compliance ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)

qqnorm(m_PC_0_ps$residuals)
qqline(m_PC_0_ps$residuals)



# response efficacy T0
m1_RE_0 <- lm(
  formula = RE_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_RE_0 <- lm(
  formula = RE_0 ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_RE_0_ps <- lm(
  formula = RE_0 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)

qqnorm(m_RE_0_ps$residuals)
qqline(m_RE_0_ps$residuals)



# behaviour index T1
m1_BEHAVIOUR_1 <- lm(
  formula = BEHAVIOUR_1 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_BEHAVIOUR_1 <- lm(
  formula = BEHAVIOUR_1 ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_BEHAVIOUR_1_ps <- lm(
  formula = BEHAVIOUR_1 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)

qqnorm(m_BEHAVIOUR_1_ps$residuals)
qqline(m_BEHAVIOUR_1_ps$residuals)



# self efficacy T1
m1_SE_1 <- lm(
  formula = SE_1 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m2_SE_1 <- lm(
  formula = SE_1 ~ as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded),
  data = reg_dataset
)

m_SE_1_ps <- lm(
  formula = SE_1 ~ as.factor(group) + as.factor(Geslacht) + Leeftijd + as.factor(Opleiding_recoded) + weight,
  data = reg_dataset
)

qqnorm(m_SE_1_ps$residuals)
...
behaviour_n <- dataset %>%
  select(
    ...
    Gedrag_2_6, Gedrag_2_7, Gedrag_2_8, Gedrag_2_9
  ) %>%
  pivot_longer(
    cols = c(everything(), - group),
    names_to = "variable"
...
item_summary_behaviour <- dataset %>%
  select(
    ...
    Gedrag_2_7, Gedrag_2_8, Gedrag_2_9
  ) %>%
  rename_all(. %>% gsub("_", ".", .)) %>%
  group_by(group) %>%
  summarise(
...
wilcox_behaviour_8 <- wilcox.test(
  formula = Gedrag_2_8 ~ group,
  data = dataset,
  estimate = TRUE,
  conf.int = TRUE
) %>% print()
...
# 9. Avoid crowds
wilcox_behaviour_9 <- wilcox.test(
  formula = Gedrag_2_9 ~ group,
  data = dataset,
  estimate = TRUE,
  conf.int = TRUE
) %>% print()
...
wilcox_behaviour <- list(
  ...
  behaviour_7 = wilcox_behaviour_7, behaviour_8 = wilcox_behaviour_8,
  behaviour_9 = wilcox_behaviour_9
); rm(
  wilcox_behaviour_1, wilcox_behaviour_2, wilcox_behaviour_3,
  wilcox_behaviour_4, wilcox_behaviour_5, wilcox_behaviour_6,
  wilcox_behaviour_7, wilcox_behaviour_8, wilcox_behaviour_9
)
...
VHS_output <- full_join(
  ...
  arrange(
    match(
      name,
      c(
        "VHS_1", "VHS_1_1", "VHS_2", "VHS_2_1", "VHS_3", "VHS_3_1",
        "VHS_4", "VHS_4_1", "VHS_5", "VHS_5_1", "VHS_6", "VHS_6_1",
        "VHS_7", "VHS_7_1", "VHS_8", "VHS_8_1", "VHS_9", "VHS_9_1",
        "VHS_10", "VHS_10_1", "VHS_11", "VHS_11_1", "VHS_12", "VHS_12_1",
        "VHS_13", "VHS_13_1"
      )
    )
  ) %>%
  as.data.frame() %>%
  print(); rm(VHS_n, VHS_pct)
